...
extension UIView{
    ...
    func maskFrom(_ fromRect:CGRect, duration:TimeInterval, complete:@escaping ()->() = {}) -> Void {
        CATransaction.begin()
        ...
        let fromPath = UIBezierPath(arcCenter: fromCenter, radius: fromRadius, startAngle: 0, endAngle: CGFloat(Double.pi) * 2, clockwise: true)
        ...
        let r3 = sqrt(r32)
        let r4 = sqrt(fromCenter.x * fromCenter.x + (fromCenter.x - viewWidth) * (fromCenter.y - viewHeight))
        let toRadius = max(max(max(r1, r2),r3),r4)
        
        let toPath = UIBezierPath(arcCenter: fromCenter, radius: toRadius, startAngle: 0, endAngle: CGFloat(Double.pi) * 2, clockwise: true)
    ...
    func maskTo(_ toRect:CGRect,duration:TimeInterval,complete:@escaping ()->() = {}) -> Void {
        CATransaction.begin()
        CATransaction.setCompletionBlock { 
            complete()
        }
        ...
        let toPath = UIBezierPath(arcCenter: toCenter, radius: toRadius, startAngle: 0, endAngle: CGFloat(Double.pi) * 2, clockwise: true)
        ...
        let fromPath = UIBezierPath(arcCenter: toCenter, radius: fromRadius, startAngle: 0, endAngle: CGFloat(Double.pi) * 2, clockwise: true)
